...
int main(int, char *[])
{
  ...
  for (int i = 0; i < 10; ++i) { custom_buffer.push_back(element); }

  // Move and copy are disabled:
  // CircularBuffer<custom_struct, 5> buffer_moved = std::move(custom_buffer);
  // CircularBuffer<custom_struct, 5> buffer_move_assigned;
  // buffer_move_assigned = std::move(buffer_moved);
  // buffer_move_assigned.push_back(std::move(element));
  return 0;
}
