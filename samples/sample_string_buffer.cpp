...
int main(int, char *[])
{
  ...
  std::cout << "Pushing back string data in buffer \n";
  for (int i = 0; i < 10; i++) {
  ...
  std::cout << "pop front executed new size: " << string_buff.size() << " new front:" << string_buff.front() << "\n";
  ...
  for (auto &it : string_buff) std::cout << "Checking range based for loop function  " << it << "\n";
  ...
  std::cout << "string moved to buffer  new size: " << string_buff.size() << " new back:" << string_buff.back() << "\n";

  // Move and copy are disabled: 

  // creating buffer from another buffer
  // CircularBuffer<std::string, 10> string_buff_copy1{ string_buff };
  // std::cout << "Buffer Copy Created, copied buffer size " << string_buff_copy1.size() << "\n";

  // copying buffer
  // CircularBuffer<std::string, 10> string_buff_copy2;
  // string_buff_copy2 = string_buff;
  // std::cout << "Buffer Copy assigned , copied buffer size " << string_buff_copy2.size() << "\n";

  // move constructing
  // CircularBuffer<std::string, 10> string_buff_move1{ std::move(string_buff) };
  // std::cout << "Buffer created with move constructor, moved buffer size " << string_buff_move1.size() << "\n";

  // move assignment
  // CircularBuffer<std::string, 10> string_buff_move2;
