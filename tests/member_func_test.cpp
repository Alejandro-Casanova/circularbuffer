...
{
protected:
  CircularBuffer<std::string, TEST_BUFFER_SIZE> test_buff;
};
...
TEST_F(CircularBufferTest, EmptyTest)
{
  ...
  test_buff.pop_front();
  EXPECT_TRUE(test_buff.empty());
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test_buff.push_back("string" + std::to_string(i));
  EXPECT_FALSE(test_buff.empty());

  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test_buff.pop_front();
  EXPECT_TRUE(test_buff.empty());
}
...
TEST_F(CircularBufferTest, FullTest)
{
  ...
  test_buff.pop_front();
  EXPECT_FALSE(test_buff.full());
  // add one element to make the buffer full
  test_buff.push_back("string" + std::to_string(TEST_BUFFER_SIZE + 2));
  EXPECT_TRUE(test_buff.full());
}
...
TEST_F(CircularBufferTest, AtFunctionTest)
{
  ...
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) EXPECT_EQ(test_buff.at(i), "string" + std::to_string(i));
  // test assignment with at function for each element
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test_buff.at(i) = "test_string";
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) EXPECT_EQ(test_buff.at(i), "test_string");
  // test out of bounds
  try {
...
TEST_F(CircularBufferTest, AccessOperatorTest)
{
  ...
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) EXPECT_EQ(test_buff[i], "string" + std::to_string(i));
  // test assignment with [] operator for each element
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test_buff[i] = "test_string";
...
TEST_F(CircularBufferTest, BeginIteratorTest)
{
  ...
  auto it = test_buff.begin();
  EXPECT_EQ(*it, "string0");
  // access with begin iterator
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) EXPECT_EQ(*(it++), "string" + std::to_string(i));

  // access with const begin iterator
  auto const_it = test_buff.begin();
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) EXPECT_EQ(*(const_it++), "string" + std::to_string(i));
  // test out of bounds
  try {
    *it = "test_string";
    FAIL() << "Expected std::out_of_range error";
...
TEST_F(CircularBufferTest, RbeginIteratorTest)
{
  ...
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test_buff.push_back("string" + std::to_string(i));
  // test first element with iterator
  auto it = test_buff.rbegin();
  // access with rbegin iterator
  for (int i = TEST_BUFFER_SIZE - 1; i >= 0; i--) EXPECT_EQ(*(it++), "string" + std::to_string(i));

  // access with const begin iterator
  auto const_it = test_buff.rbegin();
  for (int i = TEST_BUFFER_SIZE - 1; i >= 0; i--) EXPECT_EQ(*(const_it++), "string" + std::to_string(i));
  // test out of bounds
  try {
    *it = "test_string";
    FAIL() << "Expected std::out_of_range error";
...
TEST_F(CircularBufferTest, CbeginIteratorTest)
{
  ...
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test_buff.push_back("string" + std::to_string(i));
  // test first element with iterator
  auto const_it = test_buff.cbegin();
...
TEST_F(CircularBufferTest, RendIteratorTest)
{
  ...
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test_buff.push_back("string" + std::to_string(i));
  // test first element with iterator
  auto it = test_buff.rend() - 1;
  // access with rbegin iterator
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) EXPECT_EQ(*(it--), "string" + std::to_string(i));

  // access with const begin iterator
  auto const_it = test_buff.rend() - 1;
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) EXPECT_EQ(*(const_it--), "string" + std::to_string(i));
  // test out of bounds
  try {
    *it = "test_string";
