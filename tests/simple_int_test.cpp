...
{
protected:
  CircularBuffer<int, TEST_BUFFER_SIZE> test;
};
...
TEST_F(CircularBufferTest, EmptyTest)
{
  ...
  test.pop_front();
  EXPECT_TRUE(test.empty());
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test.push_back(10);
  EXPECT_FALSE(test.empty());

  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test.pop_front();
  EXPECT_TRUE(test.empty());
}
...
TEST_F(CircularBufferTest, SizeTest)
{
  ...
  while (!test.full()) test.push_back(1);
...
{
  EXPECT_EQ(0, test.size());
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test.push_back(i);
  EXPECT_EQ(TEST_BUFFER_SIZE, test.size());
  EXPECT_EQ(test.size(), test.capacity());

  for (int i = 0; i < TEST_BUFFER_SIZE - 1; i++) test.pop_front();
