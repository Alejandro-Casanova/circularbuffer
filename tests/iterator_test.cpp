...
{
protected:
  CircularBuffer<std::string,TEST_BUFFER_SIZE> test_str_buff;
  CircularBuffer<int, TEST_BUFFER_SIZE> test_int_buff;
};

TEST_F(CircularBufferTest, IteratorBasedLoopTest)
{
  // create full buffer
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test_str_buff.push_back("string" + std::to_string(i));
  int i = 0;
  for (auto it = test_str_buff.begin(); it != test_str_buff.end(); it++) EXPECT_EQ(*it, "string" + std::to_string(i++));
  // partially fill buffers
  test_str_buff.clear();
  for (int i = 0; i < TEST_BUFFER_SIZE / 2; i++) test_str_buff.push_back("string" + std::to_string(i));
...
TEST_F(CircularBufferTest, RangeBasedLoopTest)
{
  ...
  int i = 0;
  // copied elements
  for (auto buff_elem : test_str_buff) EXPECT_EQ(buff_elem, "string" + std::to_string(i++));
  // auto reference
  i = 0;
  for (auto &buff_elem : test_str_buff) EXPECT_EQ(buff_elem, "string" + std::to_string(i++));
  // auto const reference
  i = 0;
  for (const auto &buff_elem : test_str_buff) EXPECT_EQ(buff_elem, "string" + std::to_string(i++));
  // check iterators after modifications
  for (int i = 0; i < REDUCE_SIZE; i++) test_str_buff.pop_front();
...
TEST_F(CircularBufferTest, ForEachTest)
{
  ...
  int i = 0;
  for (const auto &elem : test_str_buff) EXPECT_EQ(elem, "string" + std::to_string(i++) + "modified");
}

TEST_F(CircularBufferTest, SortTest)
{
  // create full buffer
  for (int i = 0; i < TEST_BUFFER_SIZE; i++) test_int_buff.push_back(i);
  std::sort(test_int_buff.begin(), test_int_buff.end(), std::greater<int>());
  int i = 99;
  for (const auto &elem : test_int_buff) EXPECT_EQ(elem, i--);
}
...
TEST_F(CircularBufferTest, CopyUsingIterator)
{
  ...
  int i = 0;
  for (const auto &elem : test_str_buff) EXPECT_EQ(elem, buffer_copy[i++]);
  buffer_copy.clear();
